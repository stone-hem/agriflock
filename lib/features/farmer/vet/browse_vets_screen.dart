...
const List<Map<String, String>> _trialNudges = [
  ...
  },
  {
    ...
    'body': 'Join thousands of farmers boosting productivity. Start your free trial now!',
  },
  {
...
class _BrowseVetsScreenState extends State<BrowseVetsScreen> {
  ...
  void initState() {
    ...
    _maybeShowTrialNudge();
  }

  // ── Trial nudge ─────────────────────────────────────────────────────────────
  Future<void> _maybeShowTrialNudge() async {
    try {
      ...
      // Random delay between 2–5 seconds so it feels natural, not instant
      final delay = 2 + Random().nextInt(4); // 2, 3, 4, or 5 seconds
      await Future.delayed(Duration(seconds: delay));

      if (!mounted) return;

      final nudge = _trialNudges[Random().nextInt(_trialNudges.length)];
      _showTrialPopup(nudge);
    } catch (_) {
      // Never crash the screen because of the nudge
    }
  }

  void _showTrialPopup(Map<String, String> nudge) {
    late OverlayEntry entry;
    entry = PopupUtil.show(
      context: context,
      targetKey: _allVetsBtnKey, // ignored when centerOnScreen: true
      centerOnScreen: true,      // always in visible area, never behind nav bar
      showArrow: false,
      barrierDismissible: true,
      backgroundColor: Colors.white,
      child: _TrialNudgeCard(
        title: nudge['title']!,
        body: nudge['body']!,
        onDismiss: () => entry.remove(),          // "Maybe later"
        onStartTrial: () {
          entry.remove();                          // close the overlay
          context.push('/plans');                  // navigate with GoRouter
        },
      ),
    );
  }
  ...
  void _bookVet(VetFarmerRecommendation vet) =>
      context.push('/vet-order-details', extra: vet.toVetFarmer());
  ...
  Widget _buildCard(BuildContext context, VetFarmerRecommendation vet) {
    ...
    final address =
        vet.location.address?.formattedAddress ?? 'Location N/A';
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(children: [
                      Expanded(
                          child: _info(Icons.cake, 'Age', '${vet.age} yrs',
                    ...
                    Row(children: [
                      Expanded(
                          child: _info(Icons.work_history, 'Experience',
                              '${vet.yearsOfExperience} yrs',
                              Colors.orange.shade700)),
                      Expanded(
                          child: _info(Icons.task_alt, 'Jobs Done',
                              '${vet.totalJobsCompleted}',
                    ...
                    _info(Icons.group, 'Total Ratings',
                        '${vet.totalRatingsCount}', Colors.blue.shade500),
  ...
  Widget _photoFallback() => Container(
    ...
    child: Center(
        child: Icon(Icons.person, size: 70, color: Colors.green.shade300)),
  );
              ...
              if (!_isLoadingRecommended) ...[
                Row(
                  ...
                  children: [
                    ...
                    TextButton.icon(
                        onPressed: () => context.push('/all-vets'),
                        label: const Text('All Vets'),
                ...
                if (_recommendedVets.isEmpty && !_hasRecommendedError)
                  Padding(
                    ...
                    child: Column(children: [
                      ...
                      const SizedBox(height: 8),
                      Text(
                          'No veterinarians found near you yet.\nBrowse all vets below.',
                          style: TextStyle(
                  ...
                  SizedBox(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (ctx, i) =>
                          _buildCard(ctx, _recommendedVets[i]),
                    ),
                  ),

                const SizedBox(height: 24),

                // ── View All Vets button — popup anchors here ─────────────
                SizedBox(
                  key: _allVetsBtnKey,
                  width: double.infinity,
                  child: OutlinedButton.icon(
                    ...
                    style: OutlinedButton.styleFrom(
                      ...
                      side:
                      BorderSide(color: Colors.green.shade400, width: 1.8),
                      shape: RoundedRectangleBorder(
                          borderRadius: BorderRadius.circular(14)),
...
// ── Trial Nudge Card widget ───────────────────────────────────────────────────
class _TrialNudgeCard extends StatelessWidget {
  final String title;
  final String body;
  final VoidCallback onStartTrial;
  final VoidCallback onDismiss;

  const _TrialNudgeCard({
    required this.title,
    required this.body,
          ...
          Row(children: [
            ...
            TextButton(
              ...
              style: TextButton.styleFrom(
                  ...
                  padding: const EdgeInsets.symmetric(vertical: 10)),
              child: const Text('Maybe later', style: TextStyle(fontSize: 12)),
            ),
          ]),
        ],
      ),
