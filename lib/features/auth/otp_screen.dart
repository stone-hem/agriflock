...
class _OTPVerifyScreenState extends State<OTPVerifyScreen> with CodeAutoFill {
  ...
  /// Called by [CodeAutoFill] mixin when an OTP code is detected (Android).
  @override
  void codeUpdated() {
    if (!mounted) return;
    final incoming = code ?? '';
    if (incoming.length == 6) {
      _fillOtpFields(incoming);
    }
  }

  /// Distributes a 6-character string across the six individual text fields.
  void _fillOtpFields(String otp) {
    for (int i = 0; i < 6 && i < otp.length; i++) {
      _otpControllers[i].text = otp[i];
    }
    // Move focus to the last field so the user can submit / edit easily.
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        // ── 6-digit OTP fields ────────────────────────────
                        //
                        // AutofillGroup tells iOS that these fields together
                        // form a one-time-code entry, so the QuickType bar
                        // surfaces the OTP from the Messages app automatically.
                        AutofillGroup(
                          child: Row(
                            ...
                            children: List.generate(6, (index) {
                              ...
                              return SizedBox(
                                ...
                                child: TextField(
                                  ...
                                  // iOS will fill all sibling fields inside
                                  // the same AutofillGroup automatically.
                                  ...
                                  onChanged: (value) {
                                    ...
                                    // Auto-verify when last digit is typed
                                    if (index == 5 &&
                                        value.isNotEmpty &&
                                        _otpCode.length == 6) {
                                      _verifyOTP();
  ...
  void _verifyOTP() async {
    if (_otpCode.length != 6) {
      AppSnackBar.show(context,
          message: 'Please enter all 6 digits', type: SnackBarType.error);
      return;
    }

    setState(() => _isLoading = true);

    // Dismiss the iOS autofill context so the keyboard suggestion bar clears.
    ...
    try {
      ...
      if (mounted) SnackBarApiErrorHandler.handle(context, e);
      _clearOtpFields();
    } finally {
  ...
  void _clearOtpFields() {
    ...
    if (mounted && _focusNodes.isNotEmpty) _focusNodes[0].requestFocus();
  }

  @override
  void dispose() {
    // Cancel the SMS stream (Android) and clean up mixin resources.
    cancel();
